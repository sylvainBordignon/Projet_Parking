...
public class AssociationMysql {
	...
	public ArrayList<Integer> clientsUtilisantCeVehicule(String plaque) {
		ArrayList<Integer> liste = new ArrayList<>();
		try {
			PreparedStatement preparedStmt = conn.prepareStatement("Select * from Association where num_plaque_immatricule = ?");
			preparedStmt.setString(1, plaque);
			ResultSet res = preparedStmt.executeQuery();
			while(res.next()) {
	...
	public Reservation verifierReservationCorrespondantePlaqueMemeJour(String plaque) {
		try {
			...
			//		"SELECT * FROM `reservation` r, association a where r.id_client = a.id_client and a.num_plaque_immatricule = ? and DATE(r.date_debut) = DATE(now());");
			PreparedStatement preparedStmt = conn.prepareStatement(
